...
export class AdminDashboardComponent implements OnInit {
	...
	confirmDelete(project: Project) {
		this.sweetAlertBrutalService
			.confirmDelete(`PROJETO: ${project.Name}`, "EXCLUIR", "CANCELAR")
			.then(result => {
	...
	toggleFeatured(project: Project) {
		...
		this.sweetAlertBrutalService.confirm(action, message, "CONFIRMAR", "CANCELAR").then(result => {
			if (result.isConfirmed) {
				...
				this.projectService.updateProject(updatedProject).subscribe({
					next: () => {
						...
						this.sweetAlertBrutalService.toast(
							project.IsFeatured
								? "PROJETO ADICIONADO AOS DESTAQUES!"
	...
	getStatusIcon(status?: string): string {
		switch (status) {
			...
			case ProjectStatus.EmDesenvolvimento:
				return "fas fa-code";
	 ...
	 */
	openCurriculumModal(
		type: "personal" | "skill" | "experience" | "education" | "certification" | "service",
		item?: any,
	) {
	...
	deleteCurriculumItem(type: string, item: any) {
		...
		this.sweetAlertBrutalService
			.confirmDelete(`${typeNames[type] || type.toUpperCase()}: ${itemName}`, "EXCLUIR", "CANCELAR")
			.then(result => {
				if (result.isConfirmed) {
